...
def coordinate(current_passenger):
    if current_passenger.start_passenger < current_passenger.dist_passenger:
        ...
        x = 1
    height_start = (int(current_passenger.start_passenger) - 1)*4 + 60
    height_dist = (int(current_passenger.dist_passenger) - 1)*4 + 60
...
while(count<len(time_DB)):
    ...
    client.publish("elevator_pos_update", "{\"x\":\""+str(abs(cabin_awake[0].position[0])+1)+"\",\"y\":\""+str(int(floor_switch(cabin_awake[0].position[1])+1))+"\",\"timestamp\":\""+str(time.time())+"\",\"uuid\":\"0\"}")
    print cabin_awake[1].position
    client.publish("elevator_pos_update", "{\"x\":\""+str(abs(cabin_awake[1].position[0])+1)+"\",\"y\":\""+str(int(floor_switch(cabin_awake[1].position[1])+1))+"\",\"timestamp\":\""+str(time.time())+"\",\"uuid\":\"1\"}")
    print cabin_awake[2].position
    client.publish("elevator_pos_update", "{\"x\":\""+str(abs(cabin_awake[2].position[0])+1)+"\",\"y\":\""+str(int(floor_switch(cabin_awake[2].position[1])+1))+"\",\"timestamp\":\""+str(time.time())+"\",\"uuid\":\"2\"}")
